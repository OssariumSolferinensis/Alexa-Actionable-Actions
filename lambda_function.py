...
HOME_ASSISTANT_URL                = ''       # REPLACE WITH THE URL FOR YOUR HA FRONTEND
...
class HomeAssistant():
    ...
    def get_ha_state(self):
        ...
        self.response_text_no = json.loads(decoded_response)['response_text_no']
        self.response_text_yes = json.loads(decoded_response)['response_text_yes']
    ...
    def post_ha_event(self, handler_input, response: str):
        ...
        person_speak_name = ""
        return_text = ""
        if dummy_text == 'ResponseNo':
            return_text = self.response_text_no
        if dummy_text == 'ResponseYes':
            return_text = self.response_text_yes
        ...
        response = http.request(
            ...
            body=json.dumps({"event_id": self.event_id, "event_response": response, "text": self.text, "event_person_id": person_id, "response_text_no": self.response_text_no, "response_text_yes": self.response_text_yes, "return_text": return_text}).encode('utf-8')
            #body=json.dumps({"event_id": self.event_id, "event_response": response, "text": self.text, "event_person_id": person_id, "response_text_no": self.response_text_no, "response_text_yes": self.response_text_yes, "dummy_text": dummy_text, "return_text", return_text }).encode('utf-8')
        )
        ...
        return "Okay" + person_speak_name + ". " + return_text
